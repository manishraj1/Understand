        ...
        for p in prompts:
            ...
            for i in range(n):
                ...
                if "RESTATE_COST_STRUCTURE" in p:
                    # emit a mostly-correct restatement; 1-in-20 corrupted
                    ...
                    if r.random() < 0.02:
                        think = "999"
                    outs.append(f"CORRECT:+1000; THINK:-{think}/100tok; "
                                f"ANSWER:-{ansc}/word; WRONG:-{wrong}")
                elif "PANDORA" in p:
                    # near-oracle mock: read the EV hint we embed for smoke mode
                    m = re.search(r"SMOKE_ORACLE=(\w+)", p)
                    act = m.group(1) if m and r.random() < 0.9 else \
                        r.choice(["OPEN", "TAKE"])
                    outs.append(f"<think>toy reasoning {r.random():.3f}</think>"
                                f"\nDECISION: {act}")
