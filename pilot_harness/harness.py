            ...
            for i in range(n):
                ...
                if "RESTATE_COST_STRUCTURE" in p:
                    ...
                    if style < 0.3:
                        ...
                        # prose + omission of unspecified slots
                        parts = [f"A correct answer earns 1000 points"]
                        if float(think):
                            parts.append(f"thinking deducts {think} points "
                                         f"per 100 tokens")
                        if float(ansc):
                            parts.append(f"{ansc} points per word of the "
                                         f"final answer")
                        if float(wrong):
                            parts.append(f"a wrong answer deducts {wrong} "
                                         f"points")
                        body = ("<think>ok</think>\nRules: " +
                                "; ".join(parts) + ".")
                    else:
